...
class TestCreateTransaction(BaseTest):
    def test_create_transaction_success(self, client, session, common_user_authenticated, wallet_from_common_user, another_user):       
    ...
    def test_create_transaction_when_user_has_no_wallet(self, client, session, common_user_authenticated, another_user):
        response = self.request_client_post(
            client=client,
            router_url="/transaction/create/",
            authorization_token=common_user_authenticated.token,
    ...
    def test_create_transaction_with_insufficient_balance_in_wallet(self, client, session, common_user_authenticated, wallet_from_common_user, another_user):
        wallet_from_another_user = create_test_wallet(session=session, user_id=another_user.id)
    ...
    def test_create_transaction_when_users_are_not_friends(self, client, session, common_user_authenticated, another_user, wallet_from_common_user):
        wallet_from_another_user = create_test_wallet(session=session, user_id=another_user.id)
        ...
        assert response_json["error"] == "Friendship required"
